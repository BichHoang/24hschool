                    ...
                    <div class="card">
                        @if(isset($data) && $data != null && count($data)!= 0 )
                            <div class="card-header">
                                @if($data[0]->status == 1)
                                    <strong class="card-title">Danh sách đề thi đã được duyệt lên web</strong>
                                @else($data[0]->status == 2)
                                    <strong class="card-title">Danh sách đề thi đã được duyệt vào kho đề</strong>
                                @endif
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    @foreach($data as $index => $row)
                                        <tr>
                                            ...
                                            <td>
                                                @if($row->status == 1)
                                                    <a href="{{url('ctv/exam/web/detail='.$row->id)}}">
                                                        <button style="background-color: #00acc1; color: #000000">
                                                            Xem chi tiết
                                                        </button>
                                                    </a>
                                                @elseif($row->status == 2)
                                                    <a href="{{url('ctv/exam/repository/detail='.$row->id)}}">
                                                        <button style="background-color: #00acc1; color: #000000">
                                                            Xem chi tiết
                                                        </button>
                                                    </a>
                        ...
                        @else
                            ...
                            @if(isset($search_old) && $search_old != "")
